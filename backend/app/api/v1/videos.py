...
from app.services.video_processing import inspect_video
...
def upload_video(file: Annotated[UploadFile,File(...)], database: DatabaseSession,user:CurrentUser)->Video:
    ...
    return inspect_video(database,video)
...
def delete_video(video_id:UUID,database:DatabaseSession,user:CurrentUser)->None:
    video=owned(database,user,video_id); VideoStorage().delete(video.storage_key); database.delete(video); database.commit()
